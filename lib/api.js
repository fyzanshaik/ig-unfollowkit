...
export class InstagramAPI {
  ...
  async fetchMutualFollowers(targetUserId, onProgress) {
    this.#logger.info('Starting mutual_followers fetch', { targetUserId });
    const all = [];
    let maxId = null;
    let page = 0;
    let hasMore = true;
    // The UI uses page_size=12. /followers/ accepts up to 100, so we try the
    // same here and fall back to 12 on first-page failure.
    let pageSize = 100;
    let fellBack = false;
    ...
    while (hasMore) {
      ...
      page++;
      let url = `https://www.instagram.com/api/v1/friendships/${targetUserId}/mutual_followers/?page_size=${pageSize}`;
      if (maxId !== null) {
        url += `&max_id=${encodeURIComponent(maxId)}`;
      }
      ...
      try {
        ...
        if (
          ...
          this.#logger.warn(
            `mutual_followers page_size=100 failed, falling back to 12`,
            { error: err.message, targetUserId }
          );
          pageSize = 12;
          fellBack = true;
          url = `https://www.instagram.com/api/v1/friendships/${targetUserId}/mutual_followers/?page_size=${pageSize}`;
          data = await this.#fetchWithRetry(url);
        } else {
          throw err;
        }
      ...
      all.push(...users);
      maxId = data.next_max_id || null;
      hasMore = !!maxId && users.length > 0;

      this.#logger.debug(
        `mutual_followers page ${page}: got ${users.length} (total ${all.length})`,
        { targetUserId, hasMore, pageSize }
      );

      if (onProgress) onProgress(all.length, page);
      if (hasMore) await this.#sleep(REQUEST_DELAY_MS);
    }

    this.#logger.info('Finished mutual_followers fetch', {
      targetUserId,
      total: all.length,
      pages: page,
      pageSize,
